...
**Altura e inclinación juntas (`set_pose`).** `go_to_rotor()` acepta una altura final: en vez de volver a donde estaba, la maniobra acaba donde pide el comando. Si esa altura ya es más alta que la que exige el giro, sube directamente hasta ella y se ahorra el viaje de vuelta. El suelo lo pone `go_to()`, así que un destino bajo con la rejilla inclinada se queda en la altura segura. El handler rechaza antes con `rotor_busy` si hay un programa o un movimiento en marcha: la maniobra mide contra la inclinación de la que parte, y pisarla a medias la dejaría midiendo contra otra.
