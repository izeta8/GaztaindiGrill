...
Without the grill, `npm run fake-grill` (`scripts/fake-grill.mjs`) stands in for the ESP32 against a local broker on `mqtt://localhost:1883` (`MQTT_URL` overrides it). It publishes the LWT, mode and reset status, moves position and rotation for real on `set_position`, `set_rotation` and the manual commands, sends a temperature every 5 s, and answers on `status/result`. It also runs programs: it steps through them in the firmware's order, publishes the retained `status/program/current`, and honours `skip_step` and `cancel`. A temperature step just waits 5 s. With no API to list programs, `npm run fake-program -- <0|1> [cancel]` starts a demo one on that grill (left by default), or cancels it. The broker needs a WebSocket listener on 1884 and `allow_anonymous true`, and `.env.local` must leave `NEXT_PUBLIC_DEV_HOST` and the MQTT credentials unset so the page talks to localhost. With no API running, the user modal does not open and the control page still works.
...
- **3D grill visualization** (`src/components/three/`) uses `@react-three/fiber`/`drei`/`three` to render `GrillModel`/`GrillScene`. `GrillModel` looks grills up by node name (`padre_parrilla_ezkerra`/`eskubi`, and `rotor_cilindro+parrilla` for the tilt), so renaming them in Blender breaks the scene. It is also the only way to set a height on `/control`: tapping a grill opens `GrillPositionModal`, which renders the whole model with everything but that grill greyed out and see-through (corner camera for the left one, so the tilt shows; front camera for the right one, both framing the rack's full travel with margin) The height being picked is a solid copy of that grill that follows the pointer in 5 % steps, while the grill at its real height fades out. On the left grill an `Altura | Giro` selector says which of the two the drag moves, and the copy tilts with the picked angle. Two text fields over the scene hold the same targets: dragging rewrites them, and typing moves the copy, so any value can be reached exactly. A red slab marks the heights a rack at that tilt may not stay at — `minSafePosition()` in `src/utils/rotation.ts` mirrors the firmware's own floor, so both change together. "Mover" sends `set_pose` with height and tilt on the left grill, and `set_position` on the right one. In dual mode either grill opens grill 0, and the modal does not open while a program runs or the grill is offline.
