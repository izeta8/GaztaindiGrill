...
- **`defer()`** silencia ese ok automático cuando la respuesta llega más tarde (p. ej. un cambio de modo que se resuelve varias iteraciones de `loop()` después).
...
| `grill/{id}/action/movement/rotation` | `clockwise` / `counter_clockwise` / `stop` | Rotación continua. Solo la parrilla 0 tiene rotor; en la 1 devuelve `no_rotor`. |
| `grill/{id}/action/movement/set_position` | `0`–`100` | Ir a una posición concreta. |
| `grill/{id}/action/movement/set_rotation` | `0`–`359` | Ir a un ángulo concreto. Fuera de rango → `rotation_out_of_range`; sin rotor → `no_rotor`. |
...
```json
{
  ...
  "steps": [ { "time": 300, "temperature": 220, "position": 40, "rotation": 90, "action": "..." } ]
}
```
